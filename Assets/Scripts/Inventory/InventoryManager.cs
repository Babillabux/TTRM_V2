...
namespace Inventory
{
    ...
    public class InventoryManager : MonoBehaviour
    {
        ...
        public void UpdateHotbar(int id)
        {
            ...
            List<I_Content> stackToDelete = new List<I_Content>();

            foreach(var content in I_content)
            {
                if(content._quantity <= 0)
                {
                    GameObject instance = content._content;
                    stackToDelete.Add(content);
                    Destroy(instance);
                }
            }

            foreach(var stack in stackToDelete)
            {
                I_content.Remove(stack);
            }

            int selectedItemQuantity = ItemsQuantity(_takenItemId);

            if(selectedItemQuantity > 0)
            {
                _hotbarPanel.SetActive(true);
                _hotbarImage.sprite = itemData._sprite;
                _hotbarQuantity.text = selectedItemQuantity.ToString();
